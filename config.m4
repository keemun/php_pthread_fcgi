...
PHP_ARG_ENABLE(fastcgi, whether to enable fastcgi support,
dnl Make sure that the comment is aligned:
...
if test "$PHP_FASTCGI" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $FASTCGI_DIR/lib, FASTCGI_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_FASTCGILIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong fastcgi lib version or lib not found])
  dnl ],[
  dnl   -L$FASTCGI_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(FASTCGI_SHARED_LIBADD)

  PHP_NEW_EXTENSION(fastcgi, fastcgi.c connection.c, $ext_shared)
fi
